(() => {
 ...
 function hide(){window.QAReview?.clear();window.QAImports?.clear();epoch++;user=null;$('protected-content').hidden=true;$('logout').hidden=true;$('account').textContent='';if($('task-list'))$('task-list').replaceChildren();if($('draft-form'))$('draft-form').reset();if($('report-title'))$('report-title').textContent='结果展示待开放';}
 ...
 async function save(event){event.preventDefault();if(saving)return;
  ...
  try{const stamp=await requireSession();if(!active)active={id:crypto.randomUUID(),unsaved:true};const payload={product_name:name,asin:asin||null,qa_count:count};
   ...
   const {data,error}=await query.select(columns).single();if(error)throw error;if(stamp!==epoch)return;active=data;history.replaceState(null,'','/QA/?task='+data.id);$('save-state').textContent='已保存 · '+new Date(data.updated_at).toLocaleString('zh-CN');message('已保存。资料准备好后再生成问答。');window.QAImports?.open(data,client,user);window.QAReview?.open(data,client,user);await load();
 ...
 window.addEventListener('qa-sources-changed',()=>{if(active&&user)window.QAReview?.open(active,client,user);});
