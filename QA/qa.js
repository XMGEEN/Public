(() => {
 ...
 function hide(){window.QAAnswers?.clear();window.QAQuestions?.clear();window.QAReview?.clear();window.QAImports?.clear();epoch++;user=null;$('protected-content').hidden=true;$('logout').hidden=true;$('account').textContent='';if($('task-list'))$('task-list').replaceChildren();if($('draft-form'))$('draft-form').reset();if($('report-title'))$('report-title').textContent='结果展示待开放';}
 ...
 function edit(task){active=task;$('editor').hidden=false;$('product-name').value=task?.product_name||'';$('asin').value=task?.asin||'';$('qa-count').value=task?.qa_count||5;$('save-state').textContent=task?.updated_at?'已保存 · '+new Date(task.updated_at).toLocaleString('zh-CN'):'尚未保存';history.replaceState(null,'',task?'/QA/?task='+task.id:'/QA/');$('product-name').focus();window.QAImports?.open(task,client,user);window.QAReview?.open(task,client,user);window.QAQuestions?.open(task,client,user);window.QAAnswers?.open(task,client,user);}
 ...
 async function save(event){event.preventDefault();if(saving)return;
  ...
  try{const stamp=await requireSession();if(!active)active={id:crypto.randomUUID(),unsaved:true};const payload={product_name:name,asin:asin||null,qa_count:count};
   ...
   const {data,error}=await query.select(columns).single();if(error)throw error;if(stamp!==epoch)return;active=data;history.replaceState(null,'','/QA/?task='+data.id);$('save-state').textContent='已保存 · '+new Date(data.updated_at).toLocaleString('zh-CN');message('已保存。资料准备好后再生成问答。');window.QAImports?.open(data,client,user);window.QAReview?.open(data,client,user);window.QAQuestions?.open(data,client,user);window.QAAnswers?.open(data,client,user);await load();
 ...
 for(const event of ['qa-sources-changed','qa-review-changed','qa-questions-changed'])window.addEventListener(event,()=>{if(active&&user)window.QAAnswers?.open(active,client,user);});
 ...
 async function start(){
  ...
  $('new-task').onclick=()=>edit(null);$('close-editor').onclick=()=>{window.QAAnswers?.clear();$('editor').hidden=true;history.replaceState(null,'','/QA/');};$('refresh').onclick=()=>load();$('more').onclick=()=>load(true);$('draft-form').onsubmit=save;
