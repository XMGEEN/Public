(() => {
 ...
 const columns='id,product_name,asin,qa_count,market,output_language,status,created_at,updated_at';
 ...
 async function save(event){event.preventDefault();if(saving)return;
  ...
  try{const stamp=await requireSession();if(!active)active={id:crypto.randomUUID(),unsaved:true};const payload={product_name:name,asin:asin||null,qa_count:count};
   ...
   const {data,error}=await query.select(columns).single();if(error)throw error;if(stamp!==epoch)return;active=data;history.replaceState(null,'','/QA/?task='+data.id);$('save-state').textContent='已保存 · '+new Date(data.updated_at).toLocaleString('zh-CN');message('草稿已保存。未提交生成，也不会触发模型调用。');await load();
 ...
 async function start(){
  ...
  if(report){if(!taskId){message('从任务列表选择草稿即可查看状态。');return;}if(!uuid.test(taskId)){message('任务链接无效。');return;}const stamp=await requireSession();const {data:task,error}=await client.from('qa_tasks').select(columns).eq('id',taskId).eq('user_id',user.id).maybeSingle();if(stamp!==epoch)return;if(error)throw error;if(!task){message('任务不存在或不属于当前账号。');return;}$('report-title').textContent=task.product_name;$('report-state').textContent='任务状态：'+task.status+'。问答生成、报告及审核待开放。';return;}
  ...
  await load();if(taskId){if(!uuid.test(taskId)){message('草稿链接无效。');return;}const stamp=await requireSession();const {data:task,error}=await client.from('qa_tasks').select(columns).eq('id',taskId).eq('user_id',user.id).maybeSingle();if(stamp!==epoch)return;if(error)throw error;if(task)edit(task);else message('草稿不存在或不属于当前账号。');}
 }
