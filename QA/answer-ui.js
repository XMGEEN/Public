...
async function open({host,store,simulation=false}){const stamp=++epoch;host.replaceChildren();let state=null,busy=false;
 const title=el('h2',simulation?'答案草稿（本地模拟）':'答案草稿'),msg=el('p'),choices=el('fieldset'),body=el('div'),button=el('button','生成答案小样（最多2题）'),reload=el('button','刷新草稿');msg.setAttribute('role','status');
 host.append(title,el('p','生成后仅为待审核草稿。内容检查提供辅助提示，最终由你审核。'),choices,button,reload,msg,body);
 function render(){body.replaceChildren();choices.replaceChildren(el('legend','本次小样范围（最多2题）'));for(const [i,q] of (state?.available||[]).entries()){const label=el('label'),box=el('input');label.className='check-label';box.type='checkbox';box.value=q.id;box.checked=i===0||i===((state.available.length>=4)?3:1);label.append(box,document.createTextNode(q.content.zh));choices.append(label);}
 const d=state?.draft;button.textContent=d?'重新生成答案小样（最多2题）':'生成答案小样（最多2题）';if(!d){body.append(el('p','尚未生成答案。先确认问题选择，再生成小样。'));return;}
 ...
 button.onclick=async()=>{if(busy)return;const ids=[...choices.querySelectorAll('input:checked')].map(x=>x.value);if(!ids.length||ids.length>2){msg.textContent='请勾选1–2题作为本次小样';return;}if(state?.draft&&!confirm('重新生成将替换当前草稿；失败保留原稿。是否继续？'))return;
