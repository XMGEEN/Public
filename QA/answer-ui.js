...
async function open({host,store,simulation=false}){const stamp=++epoch;host.replaceChildren();let state=null,busy=false;
 ...
 host.append(title,el('p','检查完成且无明确问题即可复制和下载，无需逐条批准。'),choices,button,reload,msg,body);
 ...
 const d=state?.draft;button.textContent=d?'重新生成答案小样（最多5题）':'生成答案小样（最多5题）';if(!d){body.append(el('p','尚未生成答案。先确认问题选择，再生成小样。'));return;}
  if(window.QAResultUI){window.QAResultUI.open(body,store);return;}
 ...
 async function watch(id){busy=true;button.disabled=true;try{await store.wait(id,progress=>{if(stamp===epoch)msg.textContent=progress;},()=>stamp!==epoch);if(stamp!==epoch)return;state=await store.load();if(stamp!==epoch)return;render();msg.textContent='答案已保存，可查看检查结果并复制下载。';}catch(e){if(stamp===epoch)msg.textContent=e.message;}finally{busy=false;if(stamp===epoch)button.disabled=false;}}
...
function clear(){epoch++;window.QAResultUI?.clear();const h=document.getElementById('answers');if(h)h.replaceChildren();}
